...
def render(light_x: ti.f32, light_y: ti.f32, light_z: ti.f32, max_bounces: ti.i32):
    ...
    for i, j in pixels:
        ...
        for sample in ti.static(range(MSAA_SAMPLES)):
            ox = ti.random(ti.f32)
            oy = ti.random(ti.f32)
            u = (2.0 * ((i + ox) / WIDTH) - 1.0) * aspect * scale
            v = (2.0 * ((j + oy) / HEIGHT) - 1.0) * scale
            ray_dir = (forward + u * right + v * up).normalized()
            color += trace_ray(camera_pos, ray_dir, light_pos, max_bounces)

        color /= MSAA_SAMPLES
        pixels[i, j] = ti.sqrt(ti.min(color, ti.Vector([1.0, 1.0, 1.0])))


def main():
    window = ti.ui.Window("Lab 5 Optional - MSAA Ray Tracing", (WIDTH, HEIGHT), vsync=True)
