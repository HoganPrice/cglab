...
def refract(direction, normal, eta):
    cos_i = ti.min((-direction).dot(normal), 1.0)
    sin2_t = eta * eta * (1.0 - cos_i * cos_i)
    total_internal = sin2_t > 1.0
    refracted = ti.Vector([0.0, 0.0, 0.0])
    if total_internal:
        refracted = reflect(direction, normal).normalized()
    else:
        cos_t = ti.sqrt(1.0 - sin2_t)
        refracted = (eta * direction + (eta * cos_i - cos_t) * normal).normalized()
    return refracted, total_internal
...
def shade_diffuse(p, normal, view_dir, material, light_pos):
    ...
    shadow_hit, shadow_t, _, shadow_material = intersect_scene(shadow_origin, light_dir)
    in_shadow = shadow_hit == 1 and shadow_t < light_dist and shadow_material != 4
            ...
            if bounce < max_bounces and done == 0:
                ...
                if hit == 0:
                    ...
                    if material == 3:
                        ...
                        new_dir, total_internal = refract(ray_dir, outward_normal, eta)
                        fresnel = 0.04 + 0.96 * ti.pow(
                            1.0 - ti.abs(ray_dir.dot(outward_normal)), 5.0
                        )
                        if total_internal or fresnel > 0.5:
                            new_dir = reflect(ray_dir, normal).normalized()
                            ray_origin = p + normal * EPS
                        else:
                            ray_origin = p + outward_normal * EPS * offset_sign
                        ray_dir = new_dir
                        throughput *= ti.Vector([0.82, 0.95, 1.0])
                    else:
...
def main():
    window = ti.ui.Window("Lab 5 Optional - Glass Refraction", (WIDTH, HEIGHT), vsync=True)
